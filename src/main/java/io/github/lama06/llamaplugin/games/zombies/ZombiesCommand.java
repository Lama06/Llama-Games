...
public class ZombiesCommand extends GameCommand {
    public ZombiesCommand(GamesModule module) {
        super(module, "zombies");
        ...
        addSubCommand("doors", createCollectionConfigSubCommand(
                ...
                ZombiesGame.class,
                config -> config.doors,
                CollectionCommand.listByMappingElements(
                ...
                CollectionCommand.forbidElementsWithSameName((sender, args) -> {
                    if (!Require.argsExact(sender, args, 16)) return Optional.empty();

                    String name = args[0];

                    Optional<BlockArea> location = Require.blockArea(sender, args[1], args[2], args[3], args[4], args[5], args[6]);
                    if (location.isEmpty()) return Optional.empty();

                    Optional<BlockArea> template = Require.blockArea(sender, args[7], args[8], args[9], args[10], args[11], args[12]);
                    if (template.isEmpty()) return Optional.empty();

                    Optional<BlockPosition> activationBlock = Require.blockPosition(sender, args[13], args[14], args[15]);
                    if (activationBlock.isEmpty()) return Optional.empty();

                    String area1 = args[13];
                    String area2 = args[14];

                    OptionalInt gold = Require.integer(sender, args[15]);
                ...
                CollectionCommand.removeElementsByName(Component.text("The door was successfully removed"), Component.text("No door with this name was found"))
        ));
        ...
        addSubCommand("windows", createCollectionConfigSubCommand(
                ...
                CollectionCommand.forbidElementsWithSameName((sender, args) -> {
                    if (!Require.argsExact(sender, args, 11)) return Optional.empty();
                    ...
                    Optional<EntityPosition> spawnLocation = Require.entityPosition(sender, args[7], args[8], args[9]);
                ...
                CollectionCommand.removeElementsByName(Component.text("The window was successfully removed"), Component.text("No window with this name exists"))
        ));
        ...
        addSubCommand("weaponShops", createCollectionConfigSubCommand(
                ...
                CollectionCommand.forbidElementsWithSameName((sender, args) -> {
                    ...
                    Optional<BlockPosition> activationBlock = Require.blockPosition(sender, args[2], args[3], args[4]);
                    if (activationBlock.isEmpty()) return Optional.empty();

                    OptionalInt gold = Require.integer(sender, args[5]);
                    if (gold.isEmpty()) return Optional.empty();

                    OptionalInt refillPrice = Require.integer(sender, args[6]);
                ...
                CollectionCommand.removeElementsByName(Component.text("The weapon shop was successfully removed"), Component.text("No weapon shop with this name was found"))
        ));
        ...
        addSubCommand("additionalZombieSpawnLocations", createCollectionConfigSubCommand(
                ...
                CollectionCommand.forbidElementsWithSameName((sender, args) -> {
                    if (!Require.argsExact(sender, args, 5)) return Optional.empty();

                    String name = args[0];

                    Optional<EntityPosition> location = Require.entityPosition(sender, args[1], args[2], args[3]);
                    if (location.isEmpty()) return Optional.empty();
                ...
                CollectionCommand.removeElementsByName(Component.text("The zombie spawn location was successfully removed"), Component.text("No zombie spawn location with this name was found"))
        ));
        ...
        addSubCommand("powerSwitch", createConfigSubCommand(
                ...
                (sender, args) -> {
                    if (!Require.argsExact(sender, args, 4)) return Optional.empty();

                    Optional<BlockPosition> activationBlock = Require.blockPosition(sender, args[0], args[1], args[2]);
                    if (activationBlock.isEmpty()) return Optional.empty();

                    OptionalInt gold = Require.integer(sender, args[3]);
                    if (gold.isEmpty()) return Optional.empty();
